...
module.exports = {
    ...
    example: "```kb gexp Kawaiinex```",
    execute(message, args) {
        axios.get("https://api.slothpixel.me/api/guilds/" + args[0] + "?populatePlayers=true")
        .then(res => {
            let requestedUser = res.data.members.find(member => member.profile.username == args[0]);
        ...
        .catch(error => message.channel.send("Player or guild not found :c \n(that could happen if the nick has uppercase letters)" + error))        
    }
}